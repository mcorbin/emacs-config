...
** Appearance
   ...
   Removing menu/toolbar/scrollbar etc...
   #+begin_src emacs-lisp
     (menu-bar-mode -1)
     (tool-bar-mode -1)
     (scroll-bar-mode -1)
...
Visual line mode for all files
...
*** Key maps & binding
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (define-key vde/launcher-map "h" #'woman)    ; Help
...
(use-package racer
  ...
  (setq racer-cmd "~/prog/racer/target/release/racer")
