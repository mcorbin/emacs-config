...
** selectrum and contuls
   #+BEGIN_SRC emacs-lisp
     (defun projectile-current-buffer-name ()
      (interactive)
...
#+BEGIN_SRC emacs-lisp
      ...
      ;; pip install --user --upgrade pyflakes rope mccabe pycodestyle yapf flake8 pylint jedi black 'python-lsp-server[all]' ruff

  ;; pip install 'python-lsp-server[all]'
  ;; pip install pylsp-mypy
  ;; pip install python-lsp-black
  ;; uv tool install rope
   ;;  uv tool install mccabe
  ...
  ;; pip install python-lsp-server
    ...
    (setq lsp-pylsp-plugins-rope-autoimport-memory 't)
    ...
    (setq lsp-pylsp-plugins-pylint-enabled 'f)
    (setq lsp-pylsp-plugins-jedi-enabled 't)
    (setq lsp-pylsp-plugins-black-enabled 'f)
  ...
  (if (string= system-type "darwin")
   (progn
      (setenv "PATH" (concat (getenv "PATH") ":/Users/mathieu.corbin/Library/Python/3.13/bin/"))
      (setq exec-path (append exec-path '("/Users/mathieu.corbin/Library/Python/3.13/bin/"))))
   (progn
...
#+BEGIN_SRC emacs-lisp
  ...
  (if (string= system-type "darwin")
    (progn
        (setenv "GOPATH" "/Users/mathieu.corbin/go")
        (setenv "PATH" (concat (getenv "PATH") ":/Users/mathieu.corbin/go/bin:/usr/local/go/bin"))
        (setenv "PATH" (concat (getenv "PATH") ":/usr/local/bin"))
        (setenv "PATH" (concat (getenv "PATH") "://opt/homebrew/bin"))
        (setq exec-path (append exec-path '("/Users/mathieu.corbin/go/bin"))))
    (progn
...
* MaizAI
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     ;;  (interactive)
     ;;  (let ((provider (read-string "provider: ")))
     ;;    (setq maizaiProvider provider)
     ;;    (message "switched to provider %s" provider)))
